...
def main():
    ...
    _, val_set, n_classes = load_cls_dataset("R8", max_len=256)
    ...
    model = BERTwithBottleNeck(False, args.pooling, n_classes)
    ...
    #key_optimized_model = discrete_key_init(1,training_loader, model, device)
    trained_model = model
    ...
    for i in range(len(class_sets)):
        ...
        sampler = RandomSampler(class_sets[i], replacement=True, num_samples=100)
