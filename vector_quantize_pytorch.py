...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        print("init embed values for first head", embed[0])
    ...
    def forward(self, x):
        ...
        #print("Input shape ", shape, " flattened shape ", flatten.shape, " embed shape ", embed.shape)
        dist = -torch.cdist(flatten, embed, p = 2)

        embed_ind = gumbel_sample(dist, dim = -1, temperature = self.sample_codebook_temp)
        embed_onehot = F.one_hot(embed_ind, self.codebook_size).type(dtype)
        embed_ind = embed_ind.view(*shape[:-1])
        #print(" Embed onehot ", embed_onehot.shape, " Embed ind ", embed_ind.shape)
        quantize = batched_embedding(embed_ind, self.embed)

        if self.training:
            cluster_size = embed_onehot.sum(dim = 1)
            #print("embed_avg :", self.embed_avg.shape, "cluster size :", cluster_size.shape)
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        print("Number of heads/segments ", heads)
        print("Codebook head/segment dim ", codebook_dim)
        print("Codebook input dim/Encoder output dim ", codebook_input_dim)
        print("EMA decay ",decay)
