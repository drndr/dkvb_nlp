...
from nlp_data_utils import CustomDataset_wiki
...
key_runtimes = []
if args.baseline == 'dkv' and args.key_init == "full":
    print(args.key_init, " key_init")
    train_data = data[0]['train']
    for t,ncla in taskcla:
        if t>0:
            train_data = ConcatDataset([train_data,data[t]['train']])
        
    train_sampler = RandomSampler(train_data)
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size,pin_memory=True)

    iter_bar = tqdm(train_dataloader, desc="Key init")
    key_epoch_runtimes = appr.discrete_key_init(1,train_dataloader,iter_bar,3) # the first argument is a hack here since full key init doesnt use task-id, number of key init epochs is in the last pos
    
    key_runtimes.append(key_epoch_runtimes)
    avg_runtime = np.mean(key_runtimes)
    print('Full Key Average runtime: ', avg_runtime)
    avg_std = np.std(key_runtimes)
    print('Full Key Std runtime: ', avg_std)
    
if args.baseline == 'dkv' and args.key_init == "wiki":
    print(args.key_init, " key_init")
    
    dataset_wiki = load_dataset("wikipedia", "20220301.simple")
    
    train_df = pd.DataFrame()
    train_df['text'] = dataset_wiki['train']['text']
    
    wiki_data =  CustomDataset_wiki(train_df, 128, 'text', None)
    
    train_dataloader = DataLoader(wiki_data, batch_size=args.train_batch_size,pin_memory=True)
    
    iter_bar = tqdm(train_dataloader, desc="Key init")
    key_epoch_runtimes = appr.discrete_key_init(1,train_dataloader,iter_bar,1, is_wiki=True) # the first argument is a hack here since full key init doesnt use task-id, number of key init epochs is in the last pos
    
    key_runtimes.append(key_epoch_runtimes)
    avg_runtime = np.mean(key_runtimes)
    print('Wiki Key Average runtime: ', avg_runtime)
    avg_std = np.std(key_runtimes)
    print('Wiki Key Std runtime: ', avg_std)
# ----------------------------------------------------------------------
# Start Training.
# ----------------------------------------------------------------------
...
for t,ncla in taskcla:
    ...
    if not args.eval_only:
        if args.task in extraction_tasks:
            ...
            # Train
            print('train')
            #epoch_runtimes, avg, std, key_epoch_runtimes = appr.train(task,train_dataloader,valid_dataloader,num_train_steps,train,valid)
