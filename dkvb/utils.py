...
def load_cls_dataset(name, max_len):
    ...
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased", do_lower_case=True)  # RobertaTokenizer -'roberta-base' , DistilBertTokenizer - 'distilbert-base-uncased'
...
def load_wikipedia(max_len):
    ...
    train_set =  CustomDataset_wo_labels(train_df, tokenizer, max_len, 'text', None)
...
def load_class_increment_r52():
    # Assuming load_cls_dataset loads the "R52" dataset
    full_set = load_cls_dataset("R52", 256)[0]

    subset_list = []
    
    random.seed(25)
    pairs = [(random.sample(range(52), 2)) for _ in range(26)]
    print(pairs)
    pair_counter = 0
    for value in range(26):
        ...
        for index in range(len(full_set)):
            ...
            # Check if the field value matches the desired value
            if class_value == pairs[pair_counter][0] or class_value == pairs[pair_counter][1]:
                subset_indices.append(index)
...
def load_class_increment_20ng():
    full_set = load_cls_dataset("20ng",512)[0]
    subset_list = []
...
class CustomDataset_wo_labels(Dataset):
