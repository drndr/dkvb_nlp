...
from dkvb_t import DiscreteKeyValueBottleneck
from utils_t import load_glue_dataset, load_cls_dataset
from model_t import BERTwithBottleNeck
...
def main():
    ...
    parser.add_argument("lr_global", default = 1e-3, type=float, help="Learning rate for decoder")
    parser.add_argument("lr_values", default = 1e-1, type=float, help="Learning rate for values in bottleneck")
    ...
    print(".", args.dataset,args.epochs,args.pool_before,args.batch_size)

    if args.wandb_enabled:
        wandb.init(project=".", entity="drndr21") #name=str(args.epochs)+"e "+str(args.batch_size)+"b "+str(args.lr_global)+"Global "+str(args.lr_values)+"Values "+args.pooling+"Ptype "+str(args.pool_before)+"PB "+arg.decoder+"Decoder: ")
    ...
    model = BERTwithBottleNeck( args.decoder, 8 , args.pool_before, args.pooling, n_classes)
