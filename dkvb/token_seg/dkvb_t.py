...
from vq_ema_t import VectorQuantize
...
class DiscreteKeyValueBottleneck(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        assert decoder =='values_softmax' or (num_key_segments*dim_value)==256, 'if decoder is mlp num_key_segments*dim_value must equal to encoder output dim'
        ...
        self.values = nn.Parameter(torch.randn(num_key_segments, codebook_size, int(256/num_key_segments)))
        print("dkvbn2 init")
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.decoder =='mlp':
            ...
            memories = rearrange(memories, 'b d t -> b t d')
        #print("memories ", memories.shape)
